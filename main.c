...
int fifo(int8_t** page_table, int num_pages, int prev_page,
    ...
    //Verifica se o endereço de memoria é igual a moldura de pagina, caso seja retorna a pagina em questão         	
    	...
    	for(int j = 0; j < num_pages; j++){
    	    ...
    	    if (page_table[j][PT_FRAMEID] == i){
    		    ...
    		    if (page_table[j][PT_REFERENCE_BIT] == 0){
                	...
                	//caso contrario, dá uma segunda chance e atualiza o valor do bit de referencia para 0
               	    page_table[j][PT_REFERENCE_BIT] = 0;
               	   	//realiza a atualização da ultima pagina acessada (prev_page)
    		        prev_page = j;
    		        break;
	         	}   		
            }
          ...
          int fifo_frm, int num_frames, int clock)
{
    ...
    // Ao final retorna o menor valor encontrado
    return menor;
}
